...
- **`mcp/premiere_agent_mcp.py`** — starter stdio MCP server exposing safe, file-based tools before live Premiere control: EDL validation, review render, social package build, batch export plan, NLE mutation safety checklist, and dry-run live-bridge planning/actions. The live bridge adapter contract lives in `mcp/premiere_live_bridge.py`; the runnable local mock bridge is `mcp/premiere_bridge_server.py`; the installable CEP panel scaffold is under `premiere_bridge/` and can be symlink-installed with `scripts/install_premiere_bridge.py`; see `docs/premiere-mcp-lane.md`.
