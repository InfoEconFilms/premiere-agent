...
- **`helpers/render_preview.py <edl.json> -o <edit>/review/main.mp4`** — optional flattened MP4 review render from the same EDL. Supports hard cuts, per-range `speed` timelapses, `audio_strategy` keep/drop, target `--resolution`, `--fps`, ffprobe-backed JSON report, and a contact sheet. Use this when the user wants to watch or share a quick approval/social preview; do **not** treat it as the professional handoff or a replacement for XML.
