...
**Color is the colorist's job; this skill does not emit a grade.** The professional delivery remains XML-first: the cut lives in your NLE, and this skill ships `cut.fcpxml` + `cut.xml` + `master.srt`. A flattened MP4 review render is now available via `helpers/render_preview.py`, but it is additive for quick approval/social preview — never a replacement for the NLE handoff.

## Setup

- **`HF_TOKEN` in `.env` at project root** — only required for speaker diarization (pyannote). Skip if single-speaker.
- **`ffmpeg` + `ffprobe` on PATH.** Hard requirement. Win: `winget install Gyan.FFmpeg`. macOS: `brew install ffmpeg` for the core XML/preprocess path; for native subtitle/text filters (`subtitles`, `ass`, `drawtext`) used by styled review renders, install/link Homebrew's fuller build with `brew install ffmpeg-full && brew unlink ffmpeg && brew link ffmpeg-full`. Linux: `apt install ffmpeg`.
- **Python deps**: run `install.sh` (Linux/macOS) or `install.bat` (Windows). Installs PyTorch + the `[preprocess,fcpxml]` extras. In Hermes desktop sessions, first check which Python the health check uses (`python helpers/health.py --json` prints it); if that is the Hermes venv, run the installer with `PYTHON=/path/to/hermes/venv/bin/python bash install.sh` rather than plain `bash install.sh`, because the script otherwise prefers `python3` on PATH and may install into the system Python while the health check still fails. Optional: `pip install -e .[flash]` for Flash Attention 2 (Florence-2 speedup), `pip install -e .[diarize]` for pyannote speaker diarization, `pip install -e .[parakeet]` to pre-install the NVIDIA Parakeet NeMo fallback (only needed when ONNX Runtime can't load on the host).
- **Speech lane backends**: the default is `parakeet_onnx_lane.py` — NVIDIA Parakeet TDT 0.6B running on ONNX Runtime through a multi-session pool (TensorRT / CUDA / DirectML / CPU EP ladder, English v2 / multilingual v3 auto-routed by language). The only sanctioned alternative is `parakeet_lane.py` (NeMo torch-mode Parakeet) for hosts where ORT can't load — pin via `VIDEO_USE_SPEECH_LANE=nemo`. Output JSON shape is byte-identical between the two so cuts, diarization, and FCPXML export are lane-agnostic. `helpers/health.py --json` surfaces non-default backends in `fallbacks_active` so you know which one is running before the lane fires. **macOS/CPU pitfall:** if Parakeet ONNX returns an ONNXRuntime broadcast error like `Attempting to broadcast an axis by a dimension other than 1` on longer clips, do not trust zero-word transcript files as silence; use the Premiere project transcript as a content index, try shorter extracted audio chunks, or run the NeMo fallback in a separate/project venv. Installing `nemo_toolkit[asr]` inside Hermes' own venv can downgrade shared deps such as `packaging`, so restore Hermes pins afterwards or avoid the shared venv. **Fully air-gapped?** Pre-download the ONNX directory and set `PARAKEET_ONNX_DIR=/path/to/parakeet-onnx`; the lane skips all network calls. There is no Whisper backend in this codebase by design — Whisper hallucinates on silence and has a known word-timestamp memory regression that crashes long-form runs.
- **`yt-dlp`, `manim`, Remotion** installed only on first use.
- This skill vendors `skills/manim-video/`. Read its SKILL.md when building a Manim slot.
- When Si asks to expand Premiere Agent beyond XML rough-cut editing into agentic motion graphics, social derivatives, rendered previews, or frontier-tool parity, read `references/agentic-video-editing-frontier-2026.md` before proposing changes.
...
- **`helpers/export_fcpxml.py <edl.json> -o cut.fcpxml`** — emit editor-ready timeline files. **The single professional delivery path.** Hard-cut only right now (Hard Rule 9): the EDL's `audio_lead` / `video_tail` / `transition_in` fields are still consumed by the code path but every range must emit `0` for all three. **Default emits BOTH `cut.fcpxml` AND `cut.xml`** side-by-side from a single timeline build, because Premiere Pro and Resolve/FCP X want different XML dialects: `.fcpxml` (FCPXML 1.10+) is native to DaVinci Resolve and Final Cut Pro X, `.xml` (Final Cut Pro 7 xmeml) is native to Premiere Pro. The recipient picks whichever NLE they live in — no XtoCC conversion required for Premiere. Also calls `build_srt.py` to drop `master.srt` next to the XML for caption import. Override with `--targets {both,fcpxml,premiere}`. `--frame-rate 24` (default), 25, 29.97, 30, 60.
- **`helpers/render_preview.py <edl.json> -o <edit>/review/main.mp4`** — optional flattened MP4 review render from the same EDL. Supports hard cuts, per-range `speed` timelapses, `audio_strategy` keep/drop, target `--resolution`, `--fps`, styled subtitle burn-in (`--burn-subtitles {none,standard,minimal,bold_social,hormozi}` using `master.srt` by default), ffprobe-backed JSON report, and a contact sheet. Use this when the user wants to watch or share a quick approval/social preview; do **not** treat it as the professional handoff or a replacement for XML.
- **`helpers/build_social_package.py <edl.json> --preset youtube_shorts`** — optional derivative package builder. Writes `<edit>/social/main_captioned.mp4`, `vertical_60s.mp4`, `square.mp4`, derivative EDL JSONs, `chapters.md`, `titles.md`, `description.md`, `thumbnail_prompts.md`, and `package_report.json`. Uses the approved EDL as source of truth, duration-trims the vertical derivative, and starts with safe center-crops until smarter reframe hints exist.
- **`mcp/premiere_agent_mcp.py`** — starter stdio MCP server exposing safe, file-based tools before live Premiere control: EDL validation, review render, social package build, batch export plan, NLE mutation safety checklist, dry-run live-bridge planning/actions, sanitized `verify_premiere_connection`, read-only `get_sequence_structure`/`list_markers`, guarded editorial marker batches, review-frame export scaffolding, and SRT/VTT caption import scaffolding. Live review-frame export first tries QE still export, then AME still presets, and may fall back to short H.264 proof clips on Premiere installs with no usable still-image preset; build the contact sheet locally from the returned paths and verify MP4 proof clips have video streams before treating them as frames. The live bridge adapter contract lives in `mcp/premiere_live_bridge.py`; the runnable local mock bridge is `mcp/premiere_bridge_server.py`; the installable CEP panel scaffold is under `premiere_bridge/` and can be symlink-installed with `scripts/install_premiere_bridge.py`. The UXP frontend scaffold is under `premiere_uxp/`; if UXP Developer Tool says `Host Application specified is not available` while Premiere is running, first ensure Premiere Pro Developer Mode is enabled, then use `python3 scripts/install_premiere_uxp.py` to copy/register it in Adobe's local External UXP folder and update `~/Library/Application Support/Adobe/UXP/PluginsInfo/v1/premierepro.json`. See `docs/premiere-mcp-lane.md` and `docs/uxp-frontend-handoff-2026-08-24.md`.

**Local live-bridge quarantine note (2026-09-03):** Si moved active Premiere MCP/CEP control to `leancoderkavy/premiere-pro-mcp` via Claude. Do not re-enable this repo's `premiere-agent` MCP/CEP/UXP bridge unless Si explicitly asks. Current quarantine details live in `docs/premiere-bridge-quarantine-2026-09-03.md`.
...
7. **Preview / handoff.** Hand `cut.fcpxml` / `cut.xml` to the user to open and scrub in their NLE of choice — the `.xml` for Premiere, the `.fcpxml` for Resolve / FCP X. If the user wants a quick watchable review file, run `python helpers/render_preview.py <edit>/edl.json -o <edit>/review/main.mp4`; treat that MP4 as a review/social preview only, with XML remaining the authoritative professional edit.
...
- **Asking the user about color grade or professional delivery format.** Color is the colorist's job inside the NLE. The professional delivery remains `cut.fcpxml` + `cut.xml` + `master.srt`; the optional `render_preview.py` MP4 is only for review/social preview. Don't invent NLE delivery options that don't exist.
