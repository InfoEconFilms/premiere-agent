...
| `premiere_agent_save_project` / `undo` | write, dry-run default | Save the project to disk / undo the last action(s); require `confirm=true`. |
| `premiere_agent_set_active_sequence` / `select_item` | UI-state write, no confirm | Focus a sequence or Project-panel item; not a structural mutation. |
| `premiere_agent_create_bin` / `delete_bin` / `rename_bin` / `move_item_to_bin` | write, dry-run default | Project-panel bin management; require `confirm=true`. |
| `premiere_agent_get_item_info` / `check_offline_media` | read-only, dry-run default | Type/status info on a project item, or a recursive scan for offline media. |
